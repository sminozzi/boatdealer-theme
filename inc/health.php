...
function boatdealer_health()
{
    global $boatdealer_memory_result;
    ...
    echo '<br />';
    echo '</strong>';
    $boatdealer_memory_result = ob_get_contents();
    ob_end_clean();
    function boatdealer_add_memory_test($tests)
    {
        $tests['direct']['memory_plugin'] = array(
            'label' => __('My Memory Test', 'boatdealer'),
            'test' => 'boatdealer_memory_test',
        );
        return $tests;
    }
    $perc = $memory['usage'] / $memory['wp_limit'];
    if ($perc > .7) {
        add_filter('site_status_tests', 'boatdealer_add_memory_test');
    }
    function boatdealer_memory_test()
    {
        global $boatdealer_memory_result;
        $result = array(
            'badge' => array(
            ...
            'description' => $boatdealer_memory_result . '  ' . sprintf(
                '<p>%s</p>',
